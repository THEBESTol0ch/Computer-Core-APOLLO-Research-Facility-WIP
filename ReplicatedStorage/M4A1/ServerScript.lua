...
local BoltIn1Sound = Emitter[Tool.Name.."_BoltIn1_Sound"]
local BoltIn2Sound = Emitter[Tool.Name.."_BoltIn2_Sound"]
local MagOutSound = Emitter[Tool.Name.."_MagOut_Sound"]
...
function CreateMag()
	...
	if MagAmmoValue.Value > 0 then
		DepletedMag = game.ReplicatedStorage[Tool.Name.."Stuff"][Tool.Name.."MagLoaded"]:Clone()
	elseif MagAmmoValue.Value == 0 then
		DepletedMag = game.ReplicatedStorage[Tool.Name.."Stuff"][Tool.Name.."Mag"]:Clone()
	end
	DepletedMag.Parent = workspace.WeaponMags
	DepletedMag.CFrame = Tool.Mag.CFrame
end
...
function Shoot(Player, Target, Hit)
	...
	if MagAmmoValue.Value > 0 then
		...
		local ClonedShotSound = game.ReplicatedStorage[Tool.Name.."Stuff"][Tool.Name.."_Shot"..math.random(1, 3).."_Sound"]:Clone()
...
function DoBullets(Mode)
	if Mode == "HIDE" then
		for _, Item in pairs(Tool.Mag:GetChildren()) do
			if Item.Name == "Bullet" or Item.Name == "AnimationBullet" then
				Item.Transparency = 1
			end
		end
...
UnequipEvent.OnServerEvent:Connect(function()
	if MagAmmoValue.Value == 0 then FirstReloadValue.Value = true end
	EquipSound:Play()
end)
...
ReloadEndEvent.OnServerEvent:Connect(function()
	if InventoryAmmoValue.Value >= MagMaxAmmoValue.Value then
		InventoryAmmoValue.Value = InventoryAmmoValue.Value - MagMaxAmmoValue.Value
		MagAmmoValue.Value = MagMaxAmmoValue.Value
	elseif InventoryAmmoValue.Value < MagMaxAmmoValue.Value then
		InventoryAmmoValue.Value = 0
		MagAmmoValue.Value = InventoryAmmoValue.Value
	end
	ReloadingValue.Value = false
	FirstReloadValue.Value = false
end)
...
BoltIn1Event.OnServerEvent:Connect(function()
	BoltIn1Sound:Play()
end)

BoltIn2Event.OnServerEvent:Connect(function()
	BoltIn2Sound:Play()
end)

MagOutEvent.OnServerEvent:Connect(function()
	MagOutSound:Play()
end)
...
MagDropEvent.OnServerEvent:Connect(function()
	CreateMag()
	DoBullets("SHOW")
end)

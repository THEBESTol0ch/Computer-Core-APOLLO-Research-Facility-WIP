...
local ReloadStart_Anim = script.Parent.Parent.Animations.ReloadStart_Anim
local ReloadStart_Anim_Track = Humanoid:LoadAnimation(ReloadStart_Anim)
local Reload_Anim = script.Parent.Parent.Animations.Reload_Anim
local Reload_Anim_Track = Humanoid:LoadAnimation(Reload_Anim)
local ReloadEnd_Anim = script.Parent.Parent.Animations.ReloadEnd_Anim
local ReloadEnd_Anim_Track = Humanoid:LoadAnimation(ReloadEnd_Anim)
...
UserInputService.InputBegan:Connect(function(Input)
	...
	if InputType == Enum.UserInputType.MouseButton1 then
		if ToolEquippedValue.Value == true and ReloadingValue.Value == false and Firing == false then
			Firing = true
			Demo_Anim_Track:Stop()
			...
			if MagAmmoValue.Value > 0 then
				...
				wait(0.5)
				if MagAmmoValue.Value > 0 then YieldPlayAnimation(BoltHit_Anim_Track) end
			end
			Firing = false
		end
	end
	...
	if Input.KeyCode == Enum.KeyCode.R then
		if ToolEquippedValue.Value == true and ReloadingValue.Value == false and InventoryAmmoValue.Value > 0 and MagAmmoValue.Value < MagMaxAmmoValue.Value and Firing == false then
			Demo_Anim_Track:Stop()
			ReloadStartEvent:FireServer()
			if AimValue.Value == true then ResetAim() end
			YieldPlayAnimation(ReloadStart_Anim_Track)
			repeat
				if ReloadingValue.Value == true and MagAmmoValue.Value < MagMaxAmmoValue.Value then
					YieldPlayAnimation(Reload_Anim_Track)
					AddAmmoEvent:FireServer()
				end
			until ReloadingValue.Value == false or MagAmmoValue.Value == MagMaxAmmoValue.Value - 1 or StopReload == true
			StopReload = false
			YieldPlayAnimation(ReloadEnd_Anim_Track)
			if AmmoLoaded == false then
				YieldPlayAnimation(BoltHit_Anim_Track)
				AmmoLoaded = true
			end
...
UserInputService.InputEnded:Connect(function(Input)
	...
	if Input.KeyCode == Enum.KeyCode.R then
		wait(0.1)
		if ToolEquippedValue.Value == true and ReloadingValue.Value == true and InventoryAmmoValue.Value > 0 and MagAmmoValue.Value < MagMaxAmmoValue.Value then
			StopReload = true
		end
	end
end)
...
BoltHit_Anim_Track:GetMarkerReachedSignal("BoltOut"):Connect(function()
	BoltOutEvent:FireServer()
end)

BoltHit_Anim_Track:GetMarkerReachedSignal("BoltIn"):Connect(function()
	BoltInEvent:FireServer()
end)

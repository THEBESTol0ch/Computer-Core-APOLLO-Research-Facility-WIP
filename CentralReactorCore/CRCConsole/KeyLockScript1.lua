...
local PumpStatusValue = workspace.CoolantPump.CPU.Values.PumpStatusValue
...
PumpStatusValue.Changed:Connect(function()
	if PumpStatusValue.Value == "ONLINE" then
		GlassUp:Play()
		Key.ClickDetector.MaxActivationDistance = 10
	elseif PumpStatusValue.Value == "OFFLINE" then
		KeyToHanger(true)
	end
end)

CentralReactorCoreStatusValue.Changed:Connect(function()
	if CentralReactorCoreStatusValue.Value == "OFFLINE" and PumpStatusValue.Value == "ONLINE" then
		GlassUp:Play()
		Key.ClickDetector.MaxActivationDistance = 10
	elseif CentralReactorCoreStatusValue.Value == "POWER ON" then
		KeyToHanger(true)
	end
end)
